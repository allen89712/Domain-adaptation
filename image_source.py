...
def data_load(args): 
    ...
    # 使用ImageList來加載數據，它會自動處理文件夾路徑
    dsets["source"] = ImageList(open(args.s_dset_path).readlines(), transform=image_train())
    dsets["test"] = ImageList(open(args.test_dset_path).readlines(), transform=image_test())

    if args.trte == "val":
        # 如果是驗證模式，將源數據集分割為訓練集和驗證集
        dsize = len(dsets["source"])
        tr_size = int(0.9*dsize)
        indices = list(range(dsize))
        random.shuffle(indices)
        train_indices = indices[:tr_size]
        val_indices = indices[tr_size:]
        
        # 創建訓練集和驗證集的子集
        dsets["source_tr"] = torch.utils.data.Subset(dsets["source"], train_indices)
        dsets["source_te"] = torch.utils.data.Subset(dsets["source"], val_indices)
    else:
        # 如果不是驗證模式，使用整個源數據集作為訓練集
        dsets["source_tr"] = dsets["source"]
        dsets["source_te"] = dsets["source"]
...
if __name__ == "__main__":
    ...
    folder = 'C:/Users/User/Desktop/test0214/SHOT/object/data/'
